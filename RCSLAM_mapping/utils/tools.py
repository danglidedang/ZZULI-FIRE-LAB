...
# setup this run
def setup_experiment(config: RCSLAMConfig): 
    ...
    if config.wandb_vis_on:
        # set up wandb
        setup_wandb()
        wandb.init(project="RCSLAMMapping", config=vars(config), dir=run_path) # your own worksapce
...
def setup_optimizer(config: RCSLAMConfig, octree_feat, mlp_geo_param, mlp_sem_param, sigma_size) -> Optimizer:
    lr_cur = config.lr
    opt_setting = []
...
def setup_optimizer_traj(config: RCSLAMConfig, octree_feat, mlp_geo_param, mlp_sem_param, mlp_traj_param, sigma_size) -> Optimizer:
    lr_cur = config.lr
    opt_setting = []
    # weight_decay is for L2 regularization, only applied to MLP
    if mlp_geo_param is not None:
