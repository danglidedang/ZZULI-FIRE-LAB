...
class dataSampler():
    ...
    def sample(self, points_torch, sensor_origin_torch, normal_torch, sem_label_torch):
        ...
        clearance_sample_n = self.config.clearance_sample_n
        ...
        surface_sample_dist_ratio = surface_sample_displacement / repeated_dist + 1.0
        ...
        clearance_sample_dist_ratio = clearance_sample_displacement / repeated_dist + 1.0
        if sem_label_torch is not None:
    ...
    def sapce_carving_sample(self, points_torch, sensor_origin_torch, space_carving_level, stop_depth_thre, inter_dist_thre):
        shift_points = points_torch - sensor_origin_torch
        ...
        nugs_ridx, nugs_pidx, depth = kal.render.spc.unbatched_raytrace(octree, point_hierarchy, pyramid, prefix, virtual_origin, shift_points_directions, space_carving_level, with_exit=True)
        ...
        steps = torch.rand(mask.sum().item(), 1).cuda()
        ...
        space_carving_labels = torch.zeros(space_carving_samples.shape[0], device=self.dev)
